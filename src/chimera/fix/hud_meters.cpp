...
namespace Chimera {
    ...
    extern "C" bool hud_meter_shader_draw(RasterizerMeterParams *meter, const void *vertices) noexcept {
        ...
        psh_constants[7] = 1.0f / std::fmax(meter->gradient * 8.0f, 1.0f); //In practice, gradient is always 1.0...
        ...
        psh_constants[21] = 0.25f;
        psh_constants[22] = 0;
        psh_constants[23] = meter->flash_color_is_negative ? 1.0f : 0.0f;

        rasterizer_set_render_state(D3DRS_ANTIALIASEDLINEENABLE, TRUE);
        rasterizer_set_render_state(D3DRS_ALPHABLENDENABLE, TRUE);
        rasterizer_set_render_state(D3DRS_SRCBLEND, D3DBLEND_BLENDFACTOR);
        rasterizer_set_render_state(D3DRS_DESTBLEND, D3DBLEND_SRCALPHA);
        rasterizer_set_render_state(D3DRS_BLENDFACTOR, meter->tint_color);
